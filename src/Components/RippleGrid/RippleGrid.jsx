...
const RippleGrid = ({
    ...
    // Strict hex-to-RGB converter: only accepts "#RRGGBB"
    function hexToRgbStrict(hex) {
        const m = /^#([A-Fa-f0-9]{6})$/.exec(hex);
        if (!m) {
            throw new Error(`Invalid color "${hex}". Use exactly "#RRGGBB" format.`);
        }
...
void main() {
  ...
  gl_FragColor = vec4(color * t * finalFade * opacity, alpha);
        ...
        const uniforms = {
            ...
            gridColor: { value: hexToRgbStrict(gridColor) },
        ...
        const render = (t) => {
            ...
            const currInf = uniforms.mouseInfluence.value;
            const targInf = mouseInfluenceRef.current;
            uniforms.mouseInfluence.value += (targInf - currInf) * 0.05;
    ...
    useEffect(() => {
        ...
        const rgb = hexToRgbStrict(gridColor);
        uniformsRef.current.enableRainbow.value = enableRainbow;
        uniformsRef.current.gridColor.value = rgb;
